...
func (g *GraphHelper) InitializeGraphForUserAuth() error {
	...
	scopes := os.Getenv("MSGRAPH_USER_SCOPES")
...
func (g *GraphHelper) GetDrive() (models.Driveable, error) {
	return g.userClient.Me().Drive().Get(context.Background(),
		&users.ItemDriveRequestBuilderGetRequestConfiguration{})
}

func (g *GraphHelper) GetDrives() (models.DriveCollectionResponseable, error) {
	return g.userClient.Me().Drives().Get(context.Background(),
		&users.ItemDrivesRequestBuilderGetRequestConfiguration{})
}
